...
import { getAddressForWhat, LIQUIDITY_POOLS, LiquidityPoolInfo } from '../utils/pools_liquidity'
import { TokenAmount } from '../utils/safe-math'

import { LP_TOKENSBASE, NATIVE_SOL, TOKENSBASE } from '../utils/token_list'
import {
  commitment,
  createAmmAuthority,
  ...
  { state, getters, mutations },
  {
    async requestInfos({ commit }) {
      ...
      for (let indexAmmInfo = 0; indexAmmInfo < ammAll.length; indexAmmInfo += 1) {
        ...
        const fromCoin =
          ammInfo.coinMintAddress.toString() === TOKENSBASE.WSOL.mintAddress
            ? NATIVE_SOL.mintAddress
            : ammInfo.coinMintAddress.toString()
        const toCoin =
          ammInfo.pcMintAddress.toString() === TOKENSBASE.WSOL.mintAddress
            ? NATIVE_SOL.mintAddress
            : ammInfo.pcMintAddress.toString()
        let coin = Object.values(TOKENSBASE).find((item) => item.mintAddress === fromCoin)
        if (!coin && fromCoin !== NATIVE_SOL.mintAddress) {
          TOKENSBASE[`unknow-${ammInfo.coinMintAddress.toString()}`] = {
            symbol: 'unknown',
            name: 'unknown',
          ...
          coin = TOKENSBASE[`unknow-${ammInfo.coinMintAddress.toString()}`]
        ...
        let pc = Object.values(TOKENSBASE).find((item) => item.mintAddress === toCoin)
        if (!pc && toCoin !== NATIVE_SOL.mintAddress) {
          TOKENSBASE[`unknow-${ammInfo.pcMintAddress.toString()}`] = {
            symbol: 'unknown',
            name: 'unknown',
          ...
          pc = TOKENSBASE[`unknow-${ammInfo.pcMintAddress.toString()}`]
        ...
        if (coin.mintAddress === TOKENSBASE.WSOL.mintAddress) {
          coin.symbol = 'SOL'
          coin.name = 'SOL'
          coin.mintAddress = '11111111111111111111111111111111'
        }
        if (pc.mintAddress === TOKENSBASE.WSOL.mintAddress) {
          pc.symbol = 'SOL'
          pc.name = 'SOL'
          pc.mintAddress = '11111111111111111111111111111111'
        }
        const lp = Object.values(LP_TOKENSBASE).find((item) => item.mintAddress === ammInfo.lpMintAddress) ?? {
