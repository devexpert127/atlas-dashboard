...
export async function removeLiquidity(
  ...
  if (poolInfo.pc.mintAddress === NATIVE_SOL.mintAddress) {
    ...
    newToTokenAccount = await createAssociatedTokenAccountIfNotExist(
      toCoinAccount,
      owner,
      poolInfo.pc.mintAddress === NATIVE_SOL.mintAddress ? TOKENSBASE.WSOL.mintAddress : poolInfo.pc.mintAddress,
      transaction
    )
  }
...
export async function getLpMintInfo(conn: any, mintAddress: string, coin: any, pc: any): Promise<TokenInfo> {
  let lpInfo = Object.values(LP_TOKENSBASE).find((item) => item.mintAddress === mintAddress)
  if (!lpInfo) {
...
export async function getLpMintListDecimals(
  ...
  mintAddressInfos.forEach((item) => {
    let lpInfo = Object.values(LP_TOKENSBASE).find((itemLpToken) => itemLpToken.mintAddress === item)
    if (!lpInfo) {
...
export function getLiquidityInfoSimilar(ammIdOrMarket: string, from: string, to: string) {
  ...
  const fromCoin = from === TOKENSBASE.WSOL.mintAddress ? NATIVE_SOL.mintAddress : from
  const toCoin = to === TOKENSBASE.WSOL.mintAddress ? NATIVE_SOL.mintAddress : to
