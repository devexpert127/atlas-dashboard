...
import {
  ASSOCIATED_TOKEN_PROGRAM_ID, RENT_PROGRAM_ID, SYSTEM_PROGRAM_ID, programIds
} from './ids';
import { ACCOUNT_LAYOUT, MINT_LAYOUT } from './layouts';
import { TOKENSBASE } from './token_list';
...
export async function findAssociatedTokenAddress(walletAddress: PublicKey, tokenMintAddress: PublicKey) {
  const { publicKey } = await findProgramAddress(
    [walletAddress.toBuffer(), programIds().token.toBuffer(), tokenMintAddress.toBuffer()],
...
export async function createAssociatedTokenAccountIfNotExist(
  ...
  // @ts-ignore without ts ignore, yarn build will failed
  const ata = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, programIds().token, mint, owner, true)
...
export async function createAtaSolIfNotExistAndWrap(
  ...
  const mint = new PublicKey(TOKENSBASE.WSOL.mintAddress)
  // @ts-ignore without ts ignore, yarn build will failed
  const ata = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, programIds().token, mint, owner, true)
  if (!publicKey) {
    ...
    const wsol = await createTokenAccountIfNotExist(
      ...
      null,
      owner,
      TOKENSBASE.WSOL.mintAddress,
      amount + rent,
      transaction,
      signers
    )
    transaction.add(
      Token.createTransferInstruction(programIds().token, wsol, ata, owner, [], amount),
      Token.createCloseAccountInstruction(programIds().token, wsol, owner, owner, [])
    )
  }
}
